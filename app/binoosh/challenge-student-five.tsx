// This is a combination of 2(data of the challenge submitting without an issue) and game-screen which contains the MOCKED api call for pronunciation evaluation
...
const PRONUN_BASE = Config.FASTAPI;                  // FastAPI host for both pronunciation & emotion
...
// --- MOCKED: pronunciation evaluator ---
...
async function evaluatePronunciation(localUri: string, targetWord: string): Promise<PronunAPIResponse> {
  console.log('🎭 Using MOCKED pronunciation evaluation');
  
  // Simulate API delay
  await sleep(800 + Math.random() * 700);
  
  // Mock logic to generate realistic scores
  const randomFactor = Math.random();
  
  // Higher chance of good scores for easier words (shorter words)
  const wordLengthFactor = Math.min(1, 8 / targetWord.length);
  
  // Base score influenced by word complexity
  let baseScore = 0.3 + (randomFactor * 0.7 * wordLengthFactor);
  
  // Add some randomness but bias toward better scores for demo purposes
  if (randomFactor > 0.2) {
    baseScore = Math.min(0.95, baseScore + 0.2);
  }
  
  // Ensure score is between 0.3 and 0.95 for more realistic results
  const finalScore = Math.max(0.3, Math.min(0.95, baseScore));
  
  // Generate mock transcription (sometimes slightly wrong)
  let transcribed = targetWord;
  if (randomFactor < 0.3) {
    ...
    Object.entries(variations).forEach(([correct, wrongs]) => {
      if (targetWord.includes(correct) && Math.random() < 0.5) {
        transcribed = transcribed.replace(correct, wrongs[Math.floor(Math.random() * wrongs.length)]);
      }
    });
  }
...
export default function ChallengeGameScreen() {
  ...
  const [globalEmotion, setGlobalEmotion] = useState<string | null>(null);
  ...
  // FIX: Add ref to track the latest pseudoword URI
  const pseudoLastUriRef = useRef<string | undefined>(undefined);
  ...
  // --- REAL: emotion detector via external API ---
async function detectEmotionFromPhoto(localUri: string): Promise<string> {
  const API_URL = "https://emotion-1059215225771.asia-south1.run.app";
  
  console.log('🌐 Starting emotion detection API call...');
  ...
  const form = new FormData();
  form.append(
    "image", // Use "image" as the field name (common for image uploads)
    {
      uri: localUri,
      name,
      type: "image/jpeg",
  ...
  console.log('📤 Making POST request to emotion API...');
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Emotion API error ${res.status}: ${errorText}`);
    }

    const responseData = await res.json();
    console.log('📄 Full response data:', responseData);
    
    // Extract emotion from response - adjust field name based on API response structure
    ...
    // If the API is not ready, use mock data as fallback
    const mockEmotions = ["Happy", "Sad", "Neutral", "Sleepy"];
    const randomEmotion = mockEmotions[Math.floor(Math.random() * mockEmotions.length)];
    console.log('🔄 Using mock emotion as fallback:', randomEmotion);
...
const quickCapture = useCallback(async () => {
  try {
    console.log('🚨 STARTING CAPTURE - Camera visible:', camVisible);
    setCamVisible(true);
    ...
    console.log('✅ Picture taken - URI exists:', !!photo?.uri);
    ...
    // Call the actual emotion detection API
    const detectedEmotion = await detectEmotionFromPhoto(photo.uri);
    console.log('🎭 Emotion detected:', detectedEmotion);
    
    setEmotion(detectedEmotion);
    setImageCaptured(true);
    
    console.log('✅ CAPTURE COMPLETE - Emotion stored:', detectedEmotion);
    setGlobalEmotion(detectedEmotion);
    
  } catch (e: any) {
    console.log('❌ CAPTURE FAILED:', e.message);
    setCamVisible(false);
    // Even if capture fails, set a default emotion to continue the game
    setEmotion("Neutral");
    setImageCaptured(true);
  }
}, [camVisible]);

  // Emotion capture at 14 seconds
useEffect(() => {
  if (started && allNormalRoundsDone && !pseudoSubmitted && seconds === 17 && !imageCaptured) {
    console.log('Triggering quick emotion capture');
    setImageCaptured(true); // Mark as captured immediately to prevent duplicate triggers
    quickCapture();
  }
}, [seconds, started, allNormalRoundsDone, pseudoSubmitted, imageCaptured, quickCapture]);
  ...
  const startChallenge = useCallback(() => {
    ...
    // FIX: Reset the ref too
    pseudoLastUriRef.current = undefined;
  }, []);
  ...
  // 🧪 Evaluate & show immediate result (MOCKED API)
  const onEvaluate = useCallback(
    async (localUri: string, wordObj: Word) => {
      ...
      try {
        ...
        // 1) MOCKED pronunciation evaluation
  ...
  const finishAndSubmit = useCallback(
    async (pseudoUri?: string) => {
      ...
      try {
        ...
        const finalEmotion = globalEmotion || "Neutral";
        form.append("emotion", finalEmotion);
  ...
  // FIX: Updated onPseudoTimeUp to use the ref
  const onPseudoTimeUp = useCallback(async () => {
    ...
    try {
      ...
      let uri = pseudoLastUriRef.current; // FIX: Use ref instead of state
      if (pseudoRecording) {
        try {
          uri = await stopRecording(pseudoRecording);
          pseudoLastUriRef.current = uri; // FIX: Update ref
        } catch {}
        setPseudoRecording(null);
      }

      console.log('⏰ [PSEUDO] Final URI to submit:', uri); // FIX: Debug log
  ...
  }, [pseudoRecording, pseudoSubmitted, finishAndSubmit]); // FIX: Remove pseudoLastUri from dependencies
  ...
  // FIX: Updated onPseudoMicPressOut to update both state and ref
  const onPseudoMicPressOut = useCallback(async () => {
    if (!pseudoRecording) return;
    try {
      const uri = await stopRecording(pseudoRecording);
      setPseudoRecording(null);
      setPseudoLastUri(uri); // keep state for UI
      pseudoLastUriRef.current = uri; // FIX: Update ref for reliable access
      console.log('🎤 [PSEUDO] URI saved to state and ref:', uri);
    } catch (e) {
      Alert.alert("Recording error", String(e));
    }
  }, [pseudoRecording]);
