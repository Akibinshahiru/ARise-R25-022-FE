...
const MEDIA_BASE = process.env.EXPO_PUBLIC_MEDIA_BASE as string;        // Express media uploader (multer) for pseudoword only
...
export type LevelScore = {
  ...
  audioClipPath?: string; // URL returned by multer upload (only for pseudoword)
  hintTaken?: boolean;
};
...
// 📤 Upload to Express (multer) — used ONLY for pseudoword
async function uploadAudioViaExpress(localUri: string, fileName: string): Promise<string> {
  const form = new FormData();
  const blob = await blobFromUri(localUri);
  form.append("file", blob, fileName); // TS-safe: Blob + filename overload

  const res = await fetch(`${MEDIA_BASE}/media/upload`, { method: "POST", body: form });
  if (!res.ok) throw new Error(`Upload failed ${res.status}`);
  const j = await res.json();
  return j.publicUrl as string; // store in pseudowordRecording
}

// --- MOCK: pronunciation evaluator (no network) ---
async function mockEvaluatePronunciation(
  _localUri: string,
  targetWord: string
): Promise<{ accuracy: number; transcribed_text: string }> {
  const jitter = ((targetWord.length % 7) * 0.03); // 0 .. ~0.18
  const base = 0.75;
  const accuracy = Math.max(0.55, Math.min(1, base + jitter));
  await new Promise((r) => setTimeout(r, 250));
  return { accuracy, transcribed_text: targetWord };
}
...
export default function ChallengeGameScreen() {
  ...
  // 🧪 Evaluate & show immediate result (MOCKED; no upload here)
  const onEvaluate = useCallback(
    async (localUri: string, wordObj: Word) => {
      ...
      try {
        ...
        // 1) MOCK pronunciation (no network)
        const evalRes = await mockEvaluatePronunciation(localUri, wordObj.word);

        // 2) Compute round score from mock accuracy + time + hint factor
        const roundScore = finalScore(evalRes.accuracy, elapsedClamped, hintTaken);
        ...
        Alert.alert("Evaluation failed", String(e));
      } finally {
        setUploading(false);
      }
    },
  ...
  async (pseudoUri?: string) => {
    ...
    try {
      ...
      // Append the pseudo word audio file if it exists
      if (pseudoUri) {
        const fileName = `pseudo_${Date.now()}.m4a`;
        form.append("pseudowordRecording", {
          uri: pseudoUri,
          type: "audio/m4a",
          name: fileName,
        } as any);
      ...
      Alert.alert("Submit failed", String(e instanceof Error ? e.message : String(e)));
    }
  },
  ...
  const onPseudoTimeUp = useCallback(async () => {
    ...
    try {
      ...
      // upload if we have a clip; otherwise submit without pseudowordRecording
      let audioUrl: string | undefined = undefined;
      // if (uri) {
      //   audioUrl = await uploadAudioViaExpress(uri, `pseudo_${Date.now()}.m4a`);
      //   setPseudowordUrl(audioUrl);
      // }
